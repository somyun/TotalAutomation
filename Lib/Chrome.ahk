...
class Chrome {
	...
	__New(URLs := '', Flags := '', ChromePath := '', DebugPort := 9222, ProfilePath := '', headless:=false) {
		; Verify ChromePath
		if !ChromePath
			try FileGetShortcut A_Clipboard := A_StartMenuCommon '\Programs\Microsoft Edge.lnk', &ChromePath					;Chrome.lnk
			catch
				ChromePath := RegRead('HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\msedge.exe',,		;chrome.exe
					'C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe')												;아래 바꿈
		if !FileExist(ChromePath) && !FileExist(ChromePath := 'C:\Program Files\Google\Chrome\Application\Chrome.exe')	;위 바꿈
