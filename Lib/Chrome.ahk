...
class Chrome {
    ...
    static FindInstance(exename := 'msedge.exe', debugport := 0) {																;chrome.exe
     ...
     */
    __New(URLs := '', Flags := '', ChromePath := '', DebugPort := 9222, ProfilePath := '', headless := false) {
        ; Verify ChromePath
        if !ChromePath
            try FileGetShortcut A_Clipboard := A_StartMenuCommon '\Programs\Microsoft Edge.lnk', &ChromePath					;Chrome.lnk 대신 Microsoft Edge.lnk로 바꿈
            catch
                ChromePath := RegRead(
                    'HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\msedge.exe', ,	;chrome.exe 대신 msedge.exe를 기본값으로 바꿈
                    'C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe')												;아래와 치환함
        if !FileExist(ChromePath) && !FileExist(ChromePath := 'C:\Program Files\Google\Chrome\Application\Chrome.exe')			;위와 치환함
        ...
        Run(CliEscape(ChromePath) headless_option ' --remote-debugging-port=' this.DebugPort ' --remote-allow-origins=*'
