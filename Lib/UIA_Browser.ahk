...
class UIA_Vivaldi extends UIA_Browser {
    ...
    GetCurrentMainPaneElement() {
        ...
        this.DialogTreeWalker := UIA.CreateTreeWalker(UIA.CreateAndCondition(UIA.CreatePropertyCondition(UIA.Property.Type,
            UIA.Type.Group), UIA.CreatePropertyCondition(UIA.Property.AutomationId, "modal-bg")))
        if !this.HasOwnProp("DocumentElement") && !(this.DocumentElement := this.MainPaneElement)
            throw TargetError(
                "UIA_Browser was unable to find the Document element for browser. Make sure the browser is at least partially visible or active before calling UIA_Browser()", -
                2)
        loop 2 {
            ...
            try {
                ...
                this.ReloadButton := "", this.ReloadButtonDescription := "", this.ReloadButtonFullDescription := "",
    ...
    GetCurrentDocumentElement() {
        loop 2 {
            try {
                ...
                return this.DocumentElement := this.BrowserElement.FindElement({ Type: "Document", not: { Value: "" },
                startingElement: this.MainPaneElement })
            } catch TargetError {
    ...
    GetTabs(searchPhrase := "", matchMode := 3, caseSense := True) {
        ...
        if !searchPhrase
            return allTabs
        return UIA.Filter(allTabs, (element) => element.ElementExist({ Name: searchPhrase, matchMode: matchMode,
            caseSense: caseSense }))
    }
    ...
    GetTab(searchPhrase := "", matchMode := 3, caseSense := True) {
        ...
        if !(els := UIA.Filter(tabs, (element) => element.ElementExist({ Type: ["Text", "TabItem"], Name: searchPhrase,
            matchMode: matchMode, caseSense: caseSense, scope: 3 }))).Length
    ...
    SetURL(newUrl, navigateToNewUrl := False) => UIA_Mozilla.Prototype.GetMethod("SetURL")(this, newUrl,
        navigateToNewUrl, "{Enter}")
    ...
    GetCurrentMainPaneElement() {
        ...
        if !this.HasOwnProp("DocumentElement")
            throw TargetError(
                ...
                2)
        loop 2 {
            try this.URLEditElement := this.BrowserElement.FindFirst({ ClassName: "TopContainerView" }).FindFirstWithOptions(
                this.EditControlCondition, 2, this.BrowserElement)
            catch
            ...
            try {
                ...
                loop 2 {
                    try {
                        this.ReloadButton := UIA.TreeWalkerTrue.GetNextSiblingElement(UIA.TreeWalkerTrue.GetNextSiblingElement(
                            this.ButtonTreeWalker.GetFirstChildElement(this.NavigationBarElement)))
                        this.ReloadButtonDescription := this.ReloadButton.LegacyIAccessiblePattern.Description
                        this.ReloadButtonName := this.ReloadButton.Name
                    }
                    if (this.ReloadButtonDescription || this.ReloadButtonName)
...
class UIA_Edge extends UIA_Browser {
        ...
        loop 2 {
            try {
                if !(this.URLEditElement := this.BrowserElement.ElementExist({ Type: "Edit" })) {
                    this.ToolbarElements := this.BrowserElement.FindAll(this.ToolbarControlCondition), topCoord :=
                    10000000
                    ...
                    if this.URLEditElement.GetChildren().Length
                        this.URLEditElement := (el := this.URLEditElement.FindFirst(this.EditControlCondition)) ? el :
                            this.URLEditElement
                } else {
                    this.NavigationBarElement := UIA.CreateTreeWalker(this.ToolbarControlCondition).GetParentElement(
                        this.URLEditElement)
                }
                this.MainPaneElement := UIA.TreeWalkerTrue.GetParentElement(this.NavigationBarElement)
                if !this.NavigationBarElement
                ...
                loop 2 {
                    try {
                        this.ReloadButton := this.ButtonTreeWalker.GetNextSiblingElement(this.ButtonTreeWalker.GetNextSiblingElement(
                            this.ButtonTreeWalker.GetFirstChildElement(this.NavigationBarElement)))
    ...
    GetCurrentDocumentElement() {
        ...
        while A_TickCount < endtime
            try return this.DocumentElement := this.CurrentDocumentElement := UIA.ElementFromHandle(this.BrowserId).FindFirst(
                this.DocumentControlCondition, 4) ; ElementFromChromium works unreliably
...
class UIA_Mozilla extends UIA_Browser {
    ...
    GetCurrentMainPaneElement() {
        ...
        loop 2 {
            try {
                this.TabBarElement := this.ToolbarTreeWalker.GetNextSiblingElement(this.ToolbarTreeWalker.GetFirstChildElement(
                    this.BrowserElement))
                ...
                loop 2 {
                    try {
                        this.ReloadButton := UIA.TreeWalkerTrue.GetNextSiblingElement(UIA.TreeWalkerTrue.GetNextSiblingElement(
                            UIA.TreeWalkerTrue.GetFirstChildElement(this.NavigationBarElement)))
                        this.ReloadButtonFullDescription := this.ReloadButton.FullDescription
                        this.ReloadButtonName := this.ReloadButton.Name
                    }
                    if (this.ReloadButtonDescription || this.ReloadButtonName)
        ...
        loop 2 {
            try {
                try this.DocumentPanelElement := this.BrowserElement.FindElement({ Type: "Custom", IsOffscreen: 0 }, 2)
                    ...
                    this.DocumentPanelElement := this.BrowserElement.FindElement({ AutomationId: "tabbrowser-tabpanels" },
                    2).FindElement({ Type: "Pane", IsOffscreen: 0 }, 2)
    ...
    JSExecute(js) {
        ...
        if !this.BrowserElement.WaitElement({ Name: "Switch to multi-line editor mode (Ctrl + B)", Type: "Button" },
        5000)
            return
    ...
    CloseAlert() {
        this.GetCurrentDocumentElement()
        try UIA.TreeWalkerTrue.GetNextSiblingElement(UIA.TreeWalkerTrue.GetFirstChildElement(this.DocumentPanelElement))
        .FindElement({ AutomationId: "commonDialogWindow" }).FindFirst(this.ButtonControlCondition).Click()
    }
    ...
    CloseTab(tabElementOrName := "", matchMode := 3, caseSense := True) {
        if (tabElementOrName != "") {
            if IsObject(tabElementOrName) {
                ...
                try this.TabBarElement.FindElement({ Name: tabElementOrName, Type: "TabItem", mm: matchMode, cs: caseSense })
                .Click()
            }
        }
        this.ControlSend("{Ctrl down}w{Ctrl up}")
...
class UIA_Browser {
    InitiateUIA(wTitle := "") {
        ...
        this.DialogTreeWalker := UIA.CreateTreeWalker(UIA.CreateOrCondition(UIA.CreatePropertyCondition(UIA.Property.Type,
            UIA.Type.Custom), UIA.CreatePropertyCondition(UIA.Property.Type, UIA.Type.Window)))
    ...
    __New(wTitle := "") {
        ...
        this.BrowserType := (wExe = "chrome.exe") ? "Chrome" : (wExe = "msedge.exe") ? "Edge" : (wExe = "vivaldi.exe") ?
            "Vivaldi" : InStr(wClass, "Mozilla") ? "Mozilla" : (wExe = "brave.exe") ? "Brave" : "Unknown"
    ...
    GetCurrentMainPaneElement() {
        ...
        loop 2 {
            try this.URLEditElement := (this.BrowserType = "Chrome" && this.BrowserElement[1].Type = UIA.Property.Document
            ) ? this.BrowserElement.FindFirstWithOptions(this.EditControlCondition, 2, this.BrowserElement) : this.BrowserElement
            ...
            try {
                ...
                if !this.MainPaneElement
                    this.MainPaneElement := this.BrowserElement
                if !(this.TabBarElement := UIA.CreateTreeWalker(this.TabControlCondition).GetPreviousSiblingElement(
    ...
    ; Returns the current document/content element of the browser
    GetCurrentDocumentElement() {
        return (this.DocumentElement := this.CurrentDocumentElement := this.BrowserElement.WaitElement(this.DocumentControlCondition,
            3000))
    }
    ...
    JSGetElementPos(selector, useRenderWidgetPos := False) { ; based on code by AHK Forums user william_ahk
        ...
        RegexMatch(bounds_str, "`"x`":(\d+).?\d*?,`"y`":(\d+).?\d*?,`"width`":(\d+).?\d*?,`"height`":(\d+).?\d*?", &
            size)
    ...
    ControlClickJSElement(selector, WhichButton?, ClickCount?, Options?, useRenderWidgetPos := False) {
        ...
        ControlClick("X" (bounds.x + bounds.w // 2) " Y" (bounds.y + bounds.h // 2), this.browserId, , WhichButton?,
        ClickCount?, Options?)
    }

    ; Uses Javascript's querySelector to get a Javascript element and then Clicks that position. useRenderWidgetPos=True uses position of the Chrome_RenderWidgetHostHWND1 control to locate the position element relative to the window, otherwise it uses UIA_Browsers CurrentDocumentElement position.
    ClickJSElement(selector, WhichButton := "", ClickCount := 1, DownOrUp := "", Relative := "", useRenderWidgetPos :=
        False) {
        bounds := this.JSGetElementPos(selector, useRenderWidgetPos)
        Click((bounds.x + bounds.w / 2) " " (bounds.y + bounds.h / 2) " " WhichButton (ClickCount ? " " ClickCount : ""
    ...
    GetAlertText(closeAlert := True, timeOut := 3000) {
        ...
        while ((A_tickCount - startTime) < timeOut) {
            try {
                if IsObject(dialogEl := this.DialogTreeWalker.GetLastChildElement(this.BrowserElement)) && IsObject(
                    OKBut := dialogEl.FindFirst(this.ButtonControlCondition))
                    break
            }
            Sleep 100
        ...
        try
            text := this.BrowserType = "Edge" ? dialogEl.FindFirstWithOptions(this.TextControlCondition, 2, dialogEl).Name :
                dialogEl.FindFirst(this.TextControlCondition).Name
        if closeAlert {
            Sleep 500
            try OKBut.Click()
        }
    ...
    WaitTitleChange(targetTitle := "", timeOut := -1) {
        ...
        while ((((A_TickCount - startTime) < timeOut) || (timeOut = -1)) && (targetTitle ? !UIA_Browser.CompareTitles(
            targetTitle, newTitle) : (origTitle == newTitle))) {
            Sleep 200
    ...
    WaitPageLoad(targetTitle := "", timeOut := -1, sleepAfter := 500, titleMatchMode := "", titleCaseSensitive := False
        ...
        local legacyPattern := "", startTime := A_TickCount, wTitle := "", ReloadButtonName := "",
            ReloadButtonDescription := "", ReloadButtonFullDescription := ""
        ...
        while ((A_TickCount - startTime) < timeOut) || (timeOut = -1) {
            ...
            && (this.ReloadButtonDescription && legacyPattern ? InStr(ReloadButtonDescription, this.ReloadButtonDescription
            ) : 1)
            && (this.ReloadButtonFullDescription ? InStr(ReloadButtonFullDescription, this.ReloadButtonFullDescription) :
                1)))
    ...
    ; Presses the Forward button
    Forward() {
        this.ButtonTreeWalker.GetNextSiblingElement(this.ButtonTreeWalker.GetFirstChildElement(this.NavigationBarElement
        )).Click()
    }
    ...
    ; Sets the URL bar to newUrl, optionally also navigates to it if navigateToNewUrl=True
    SetURL(newUrl, navigateToNewUrl := False) {
        this.URLEditElement.ValuePattern.SetValue("javascript:window.location.href='" newUrl "';")
    ...
    GetTabs(searchPhrase := "", matchMode := 3, caseSense := True) {
        return (searchPhrase == "") ? this.TabBarElement.FindAll(UIA.CreatePropertyCondition(UIA.Property.Type, UIA.Type
            .TabItem)) : this.TabBarElement.FindElements({ Name: searchPhrase, Type: "TabItem", mm: matchMode, cs: caseSense })
    }
    ...
    ; Returns a tab element with text of searchPhrase, or if empty then the currently selected tab. matchMode follows SetTitleMatchMode scheme: 1=tab name must must start with tabName; 2=can contain anywhere; 3=exact match; RegEx
    GetTab(searchPhrase := "", matchMode := 3, caseSense := True) {
        return (searchPhrase == "") ? this.TabBarElement.FindElement({ Type: "TabItem", SelectionItemIsSelected: 1 }) :
            this.TabBarElement.FindElement(searchPhrase is Integer ? { Type: "TabItem", i: searchPhrase } : { Name: searchPhrase,
                Type: "TabItem", mm: matchMode, cs: caseSense })
    }
    ; Checks whether a tab element with text of searchPhrase exists: if a matching tab is found then the element is returned, otherwise 0 is returned. matchMode follows SetTitleMatchMode scheme: 1=tab name must must start with tabName; 2=can contain anywhere; 3=exact match; RegEx
    TabExist(searchPhrase := "", matchMode := 3, caseSense := True) {
    ...
    IsBrowserVisible() {
        ...
        if ((this.BrowserId == this.WindowFromPoint(X, Y)) || (this.BrowserId == this.WindowFromPoint(X, Y + H - 1)) ||
        (this.BrowserId == this.WindowFromPoint(X + W - 1, Y)) || (this.BrowserId == this.WindowFromPoint(X + W - 1, Y +
            H - 1)))
            return True
        return False
    }
    ...
    static CompareTitles(compareTitle, winTitle, matchMode := "", caseSense := True) => UIA_Browser.StrCompare(winTitle,
        compareTitle, matchMode ? matchMode : A_TitleMatchMode, caseSense)
