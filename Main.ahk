...
; 컴파일러 지시문
; ==============================================================================
;@Ahk2Exe-SetVersion 3.5.5.0
;@Ahk2Exe-SetProductVersion v3.5.5
;@Ahk2Exe-SetDescription 통합자동화
; ==============================================================================
; ==============================================================================
; 초기화
; ==============================================================================
global AppVersion := "v3.5.5"
global wvc := ""
global wv := ""
global MainGui := ""
global LoadingGui := ""
...
OnWebMessage(sender, args) {
    ...
    if (command == "ready") {
        ...
        try {
            ...
            if (runWebPW != "" && runPW2 != "") {
                ...
                ;핸들을 잃어버린 좀비 세션BG.exe가 있다면 이름으로 찾아 전부 종료
                while ProcessExist("세션BG.exe") {
                    ProcessClose("세션BG.exe")
                }
                ...
                if (BackgroundProcessManager.Launch(cmdLine, OnSessionBGOutput)) {
                    ...
                    wv.PostWebMessageAsJson(JSON.stringify(Map("type", "sessionStatus", "state", "acquiring")))
                } else {
                    LogDebug("세션BG 프로세스 실행 실패")
                    wv.PostWebMessageAsJson(JSON.stringify(Map("type", "sessionStatus", "state", "error", "message",
                        "프로세스 실행 실패")))
                    LogDebug("[오류] 로그인 프로세스 실행 실패 (MsgBox 표시)")
    ...
    ; --- 6. 엑셀 승인 정보 요청 ---
        ...
        bringApproval(driverName) ; 차량일지.ahk 함수 호출
    }
        ...
        if (msg.Has("passwordsChanged") && msg["passwordsChanged"]) {
            ...
            if (uid != "") {
                ...
                if (userRoot.Has("profile")) {
                    ...
                    if (runWebPW != "" && runPW2 != "") {
                        LogDebug("비밀번호 변경 감지: 세션BG 재시작 (ID: " uid ")")
                        exePath := A_ScriptDir "\세션BG.exe"
                        BackgroundProcessManager.Cleanup()
                        while ProcessExist("세션BG.exe") {
                            ProcessClose("세션BG.exe")
                        ...
                        cmdLine := Format('"{1}" "{2}" "{3}" "{4}"', exePath, uid, runWebPW, runPW2)
                        if (BackgroundProcessManager.Launch(cmdLine, OnSessionBGOutput)) {
                            LogDebug("세션BG 프로세스 재실행 성공")
                            wv.PostWebMessageAsJson(JSON.stringify(Map("type", "sessionStatus", "state", "acquiring")))
                        } else {
                            LogDebug("세션BG 프로세스 재실행 실패")
                            wv.PostWebMessageAsJson(JSON.stringify(Map("type", "sessionStatus", "state", "error",
                                "message", "세션 프로세스 재실행 실패")))
                        }
                    }
                }
            }
...
OnSessionBGOutput(line) {
    ...
    try {
        ...
        if (SubStr(Trim(line), 1, 1) == "{") {
            ...
            if (wv) {
                wv.PostWebMessageAsJson(JSON.stringify(Map("type", "headlessReady")))
                wv.PostWebMessageAsJson(JSON.stringify(Map("type", "sessionStatus", "state", "ready")))
            }

            ; 근태 조회 (세션 확보 직후 자동 실행)
            RequestGuntaeData()

            ; 세션 유지를 위한 주기적 ERP 새로고침 타이머 (1시간 = 3600000ms)
            SetTimer AutoRefreshERPOrder, 3600000
