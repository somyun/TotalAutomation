    ...
    Run() {
        ...
        if this.data["chkCreate"] {
            if this.EnsureReady("WorkLog_Create")
                ...
                LogDebug("[오류] 일지 생성 준비 실패 | WebAutoLogin.EnsureReady 반환=false")
        ...
        if this.data["chkGeneralWork"] {
            if this.cUIA or this.EnsureReady("WorkLog_View")
                ...
                LogDebug("[오류] 일지 조회 준비 실패 (일반업무) | cUIA 없음, WebAutoLogin.EnsureReady 반환=false")
        ...
        if this.data["chkSafety"] {
            if this.cUIA or this.EnsureReady("WorkLog_View")
                ...
                LogDebug("[오류] 일지 조회 준비 실패 (안전관리) | cUIA 없음, WebAutoLogin.EnsureReady 반환=false")
                MsgBox("일지 조회 준비에 실패하였습니다.", "알림", "Iconx")
                StopMacro()
                return false
            }
        ...
        if this.data["chkDriving"] {
            if this.cUIA or this.EnsureReady("WorkLog_View")
                ...
                LogDebug("[오류] 일지 조회 준비 실패 (운전적합성) | cUIA 없음, WebAutoLogin.EnsureReady 반환=false")
                MsgBox("일지 조회 준비에 실패하였습니다.", "알림", "Iconx")
                StopMacro()
                return false
            }
